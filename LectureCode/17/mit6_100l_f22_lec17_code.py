...
class Vehicle:
    def __init__(self, n_wheels, n_occupants, color):
        self.n_wheels = n_wheels
        self.n_occupants = n_occupants
        self.color = color

    def __str__(self):
        return f"Vehicle with {self.n_wheels} wheels, " \
               f"{self.n_occupants} occupants, color '{self.color}'"
v = Vehicle(4,3,'red')
print(v)
...
class Vehicle(object):
    ...
    def add_n_occupants(self, n):
        if self.occ + n <= self.max_occupancy:
            ...
            raise ValueError("You cannot add more occupants more than 5")
        return self.occ
